...
void ia_main_menu() {
    ...
    ia_supply_type_menu();                                                      //Call the next menu function
...
void ia_supply_type_menu() {
    ...
    ia_supply_voltage_menu();                                                   //Call the next menu function
}
...
void ia_supply_voltage_menu() {
    ...
    ia_gain_menu();                                                             //Call the next menu function
}
...
void ia_gain_menu() {
    ...
    ia_input_z_menu();                                                          //Call the next menu function
}
...
void ia_input_z_menu() {
    ...
    ia_freq_menu();                                                             //Call the next menu function
}
...
void ia_freq_menu() {
    ...
    ia_calculations_main_menu();                                                //Call the next menu function
}
...
void ia_calculations_main_menu() {
    ...
    user_cont();                                                                //Wait for enter to be pressed
    ia_more_data();                                                             //Calls next menu function
}
...
void ia_more_data() {
    ...
    switch(input) {                                                             //Switch statement for different input options
        case 1: ia_detailed_calculations();                                     //Each case calls the respective menu function
            break;
...
void ia_detailed_calculations() {
    ...
    user_cont();                                                                //Wait for enter
    ia_more_data();                                                             //Call next menu function, returning to the more data page
}
//Function to save component values to a text file
void ia_save_to_text() {
    std::ofstream file;                                                         //Create stream object

    print_menu("Save Calculations to Text File", "Enter the filepath to create the file:", emptyList);  //Print menu with no items
    while(1){
        std::cout<< "ENTER PATH:";                                              //Prompt user input
        std::string path;
        std::cin >> path;                                                       //Store input in path
        file.open(path + "/Inverting Amplifier Components.txt");                //Open file at given path

        if(file.is_open()) {                                                    //If the file is open, break the while loop
            break;
        }
        else {
    ...
    ia_more_data();                                                             //Call the next menu function
}
//Function to generate a text file containing Falstad circuit code
void ia_generate_circuit() {
    std::ofstream file;                                                         //Create stream object

    print_menu("Generate Falstad Circuit", "Enter the filepath to create the file:", emptyList);    //Prints menu
    while(1){
        ...
        if(file.is_open()) {
            break;                                                              //If file opens successfully, break while loop
        }
        else {
            std::cerr << "COULD NOT OPEN LOCATION\n";                           //Else print error message
        }
...
void ia_generate_wave() {
    ...
    if(input == 2) {                                                            //If input is 2, get file path for custom wave and set amplitude to 1
        ...
        if(input == 1) {
            filePath = ".\\Resources\\SINE_WAVE.csv";                           //If sine wave is selected, set path to SINE_WAVE.csv
        }
        else if(input == 2) {
            filePath = ".\\Resources\\SQUARE_WAVE.csv";                         //If square wave is selected, set path to SQUARE_WAVE.csv
        }
        else if(input == 3) {
...
//            filePath = ".\\Resources\\RAMP_DN_WAVE.csv";
    ...
    print_menu("Save Location", "Enter the path of the output file:", emptyList);   //Print menu
    std::cout << "ENTER PATH:";
    std::string outPath;
...
void ia_move_on() {
    ...
    switch (input) {
        case 1: ia_main_menu();                                                 //If input is 1, go to the main menu of the op-amp calculator
            break;
